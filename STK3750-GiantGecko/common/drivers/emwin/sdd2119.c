/***************************************************************************//**
 * @file
 * @brief SDD2119 LCD controller driver
...
#ifdef SDD2119_REGISTER_ACCESS_HOOKS
...
#ifdef SDD2119_REGISTER_ACCESS_HOOKS
static void SDD2119_WriteRegister(uint16_t reg)
{
  *current_context->commandAddr = reg;
  return;
}

static uint16_t SDD2119_ReadRegister(uint16_t reg)
{
  *current_context->commandAddr = reg;
  return *current_context->dataAddr;
}

static void SDD2119_WriteData(uint16_t val)
{
  *current_context->dataAddr = val;
}

static uint16_t SDD2119_ReadData(void)
{
  return *current_context->dataAddr;
}
#endif

inline static void SDD2119_SetCurrentPosition(DRIVER_CONTEXT *context, int x, int y)
{
  /* Set pixel position */
...
inline static void SDD2119_PrepareDataAccess(DRIVER_CONTEXT *context)
{
...
******************************************************************************/
void SDD2119_Initialize(GUI_DEVICE * pDevice)
{
  uint16_t data;
 ...
 *****************************************************************************/
static void SDD2119_SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex)
{
  SDD2119_SetCurrentPosition((DRIVER_CONTEXT *)pDevice->u.pContext, x, y);
  SDD2119_PrepareDataAccess((DRIVER_CONTEXT *)pDevice->u.pContext);
 ...
 *****************************************************************************/
static unsigned int SDD2119_GetPixelIndex(GUI_DEVICE * pDevice, int x, int y)
{
 ...
 *   compability with other display drivers.
 *****************************************************************************/
static void SDD2119_XorPixel(GUI_DEVICE * pDevice, int x, int y) {
  LCD_PIXELINDEX PixelIndex;
  LCD_PIXELINDEX IndexMask;

  PixelIndex = SDD2119_GetPixelIndex(pDevice, x, y);
 ...
 *   Argument pDevice is present for compability with other display drivers.
 *****************************************************************************/
static void SDD2119_FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
  uint16_t PixelIndex;
  DRIVER_CONTEXT *context = (DRIVER_CONTEXT *)pDevice->u.pContext;
  ...
  SDD2119_SetCurrentPosition(context, xl, yl);
  PixelIndex = LCD__GetColorIndex();
  SDD2119_PrepareDataAccess(context);
  while(size--)
 ...
 *   To achieve this functionality SDD2119_FillRect function is executed, so as a side
 *   effect, function changes clipping area to entire display.
 *   Argument pDevice is present for compability with other display drivers.
 *****************************************************************************/
static void SDD2119_DrawHLine(GUI_DEVICE * pDevice, int x0, int y,  int x1)
{
  SDD2119_FillRect(pDevice, x0, y, x1, y);
}
 ...
 *   Argument pDevice is present for compability with other display drivers.
 *****************************************************************************/
static void SDD2119_DrawVLine(GUI_DEVICE * pDevice, int x, int y0,  int y1)
{
  SDD2119_FillRect(pDevice, x, y0, x, y1);
}
/**************************************************************************//**
 * @brief
 ...
 *   Argument pDevice is present for compability with other display drivers.
 *****************************************************************************/
static void SDD2119_HorizontalLine1bpp(GUI_DEVICE * pDevice, unsigned x, unsigned y, uint8_t const GUI_UNI_PTR * p, int Diff, int xsize, const LCD_PIXELINDEX * pTrans) {
 ...
 *   Argument pDevice is present for compability with other display drivers.
 *****************************************************************************/
static void  SDD2119_HorizontalLine2bpp(GUI_DEVICE * pDevice, int x, int y, uint8_t const GUI_UNI_PTR * p, int Diff, int xsize, const LCD_PIXELINDEX * pTrans) {
  ...
  case 0:
    if (pTrans) {
      do {
        ...
        SDD2119_SetPixelIndex(pDevice, x++, y, PixelIndex);
        if (++CurrentPixel == 4) {
          CurrentPixel = 0;
          Pixels = *(++p);
        }
      ...
      do {
        Shift = (3 - CurrentPixel) << 1;
        Index = (Pixels & (0xC0 >> (6 - Shift))) >> Shift;
        SDD2119_SetPixelIndex(pDevice, x++, y, Index);
        if (++CurrentPixel == 4) {
          CurrentPixel = 0;
          Pixels = *(++p);
        }
 ...
 *   Argument pDevice is present for compability with other display drivers.
 *****************************************************************************/
static void  SDD2119_HorizontalLine4bpp(GUI_DEVICE * pDevice, int x, int y, uint8_t const GUI_UNI_PTR * p, int Diff, int xsize, const LCD_PIXELINDEX * pTrans) {
  LCD_PIXELINDEX Pixels, PixelIndex;
  int CurrentPixel, Shift, Index;
  Pixels       = *p;
  CurrentPixel = Diff;
 ...
 *   Argument pDevice is present for compability with other display drivers.
 *****************************************************************************/
static void  SDD2119_HorizontalLine8bpp(GUI_DEVICE * pDevice, int x, int y, uint8_t const GUI_UNI_PTR * p, int xsize, const LCD_PIXELINDEX * pTrans) {
 ...
 *   Argument pDevice is present for compability with other display drivers.
 *****************************************************************************/
static void SDD2119_HorizontalLine16bpp(GUI_DEVICE * pDevice, int x, int y, uint16_t const GUI_UNI_PTR *p, int xsize) {
 ...
 *   Argument pDevice is present for compability with other display drivers.
 *****************************************************************************/
static void SDD2119_DrawBitmap(GUI_DEVICE * pDevice, int x0, int y0,
                       int xSize, int ySize,
                       int BitsPerPixel,
                       int BytesPerLine,
  ...
  case 1:
    for (i = 0; i < ySize; i++) {
      SDD2119_HorizontalLine1bpp(pDevice, x0, i + y0, pData, Diff, xSize, pTrans);
      pData += BytesPerLine;
    }
    break;
  case 2:
    for (i = 0; i < ySize; i++) {
      SDD2119_HorizontalLine2bpp(pDevice, x0, i + y0, pData, Diff, xSize, pTrans);
      pData += BytesPerLine;
    }
    break;
  case 4:
    for (i = 0; i < ySize; i++) {
      SDD2119_HorizontalLine4bpp(pDevice, x0, i + y0, pData, Diff, xSize, pTrans);
      pData += BytesPerLine;
    }
    break;
  case 8:
    for (i = 0; i < ySize; i++) {
      SDD2119_HorizontalLine8bpp(pDevice, x0, i + y0, pData, xSize, pTrans);
      pData += BytesPerLine;
    }
    break;
  case 16:
    for (i = 0; i < ySize; i++) {
      SDD2119_HorizontalLine16bpp(pDevice, x0, i + y0, (const uint16_t *)pData, xSize);
      pData += BytesPerLine;
    }
    break;
  }
 ...
 *****************************************************************************/
static I32 SDD2119_GetDevProp(GUI_DEVICE * pDevice, int Index) {
...
*****************************************************************************/
static void * SDD2119_GetDevData(GUI_DEVICE * pDevice, int Index) {
...
#ifdef SDD2119_REGISTER_ACCESS_HOOKS
      context->pfWriteRegister = SDD2119_WriteRegister;
      context->pfReadRegister = SDD2119_ReadRegister;
      context->pfWriteData = SDD2119_WriteData;
      context->pfReadData = SDD2119_ReadData;
#endif
      context->pfInitialize = SDD2119_Initialize;
 ...
 *   defined rectangle.
 *****************************************************************************/
static void SDD2119_ReadRectangle(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1, LCD_PIXELINDEX * pBuffer) {
  int x, y;
  uint16_t * p;
  ...
  for(y=y0;y<=y1;y++)
     for(x=x0;x<=x1;x++)
       *p++ = SDD2119_GetPixelIndex(pDevice, x, y);
}
 ...
 *****************************************************************************/
static void (* SDD2119_GetDevFunc(GUI_DEVICE ** ppDevice, int Index))(void) {
  ...
  case LCD_DEVFUNC_READRECT:
    return (void (*)(void))SDD2119_ReadRectangle;
...
const GUI_DEVICE_API GUIDRV_SSD2119 =
{
  ...
  SDD2119_DrawVLine,
  SDD2119_FillRect,
  SDD2119_GetPixelIndex,
  SDD2119_SetPixelIndex,
  SDD2119_XorPixel,
  _SetOrg,
  SDD2119_GetDevFunc,
  SDD2119_GetDevProp,
