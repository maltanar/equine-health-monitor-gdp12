...
\TODO{rephrase} - we actually do not use pure OOP methodology, we use a mixture of OOP and procedural to get the best of both worlds (encapsulation and polymorphism from C++, and C where small simple quick drivers/structures are needed).
The monitoring device software was developed using C++ with IAR Embedded Workshop for ARM as the development environment. C++ classes were created to hide implementation details for most peripherals and subsystems, using the emlib peripheral library from Energy Micro was used to interact with the EFM32 peripheral. 
...
\subsubsection{Task Scheduling and Sleep Management}
...
\subsubsection{Port Configuration and Access System}
\label{sec:port_configuration}
\begin{figure}
\includegraphics[width=\textwidth]{Images/USARTManager}
\caption{Class diagram: USART Manager}
\label{fig:usart_manager}
\end{figure}
...
\subsubsection{Automated Data Acquisition: DMA and PRS}
\TODO{ Jose:}
why? decreases load on CPU and allows more sleep, guarantees better timings for time-critical/BW-critical operations
...
The data presentation module is written in Python and uses flask\footnote{\url{http://flask.pocoo.org}},  a lightweight Python web framework to generate the web interface dynamically. 
